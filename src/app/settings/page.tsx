...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return
      const { data } = await supabase.from('bot_configs').select('*').eq('user_id', user.id).single()
  ...
  async function save() {
    ...
    try {
      ...
      // Only update private key if a new one was entered
      if (privateKey) {
        // In production: encrypt before storing. For now store as-is (add encryption layer).
        update.hl_private_key_enc = privateKey
      }

      const { error } = await supabase.from('bot_configs')
    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <section className="card mb-4">
        ...
        <div className="bg-blue/5 border border-blue/20 rounded-lg px-4 py-3 mb-5 text-xs text-muted leading-relaxed">
          ...
          <a href="https://app.hyperliquid.xyz/API" target="_blank" rel="noopener" className="text-green hover:underline">
            app.hyperliquid.xyz/API
          </a>. APEX can only place trades — withdrawal is architecturally impossible via API keys.
        </div>
        <div className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-subtle mt-1.5">Leave blank to keep existing key. The API sub-wallet cannot withdraw funds.</p>
          </div>
            ...
            <div className="flex items-center gap-4 ml-2">
              {['testnet', 'mainnet'].map(net => (
                <label key={net} className="flex items-center gap-2 cursor-pointer">
                  ...
                  {net === 'testnet' && <span className="badge-gold">Recommended first</span>}
                </label>
              ))}
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select className="input"
              ...
              {[0.05,0.10,0.15,0.20].map(v => <option key={v} value={v}>{(v*100).toFixed(0)}% of equity</option>)}
            </select>
          </div>
          <div>
            ...
            <select className="input"
              ...
              {[0.03,0.05,0.07,0.10].map(v => <option key={v} value={v}>{(v*100).toFixed(0)}% — circuit breaker</option>)}
            </select>
          </div>
          <div>
      ...
      <section className="card">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <input type="checkbox" id="emailAlerts" className="accent-green"
              checked={config.email_alerts ?? true}
              onChange={e => setConfig(c => ({ ...c, email_alerts: e.target.checked }))} />
            <label htmlFor="emailAlerts" className="text-sm cursor-pointer">Email alerts on trade open/close</label>
          </div>
        </div>
      </section>
    </div>
